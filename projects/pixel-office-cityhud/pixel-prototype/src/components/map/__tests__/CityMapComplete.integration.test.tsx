...
describe('CityMapComplete Integration', () => {
  ...
  it('shows tooltip when hovering', () => {
    ...
    expect(container.querySelector('[data-testid="building-tooltip"]')).not.toBeInTheDocument()
    ...
    // Verify hover state was updated
    const hovered = useMapStore.getState().hovered
    expect(hovered.type).toBe('building')
    expect(hovered.id).toBe('test-compute')

    // The tooltip should now be visible in the DOM
    const tooltip = container.querySelector('[data-testid="building-tooltip"]')
